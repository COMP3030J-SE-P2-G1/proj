...
async function demoInitDynamicChart() {
    ...
    Chart.initDynamicChart<TimelyArrayData, string>(
        ...
        async state => {
            ...
            return PROFILE_API.getSolar(profile.id, startTime, endTime, null, "day") as Promise<TimelyArrayData[]>;
        },
...
function demoInitSolarLineChart() {
    ...
    if (elm) Chart.initSolarChart(elm, 1, gStartTime, gEndTime, "day", {
...
function demoInitElectricityPriceLineChart() {
    ...
    if (elm) Chart.initElectricityPriceChart(elm, gStartTime, gEndTime, null, "day", {
